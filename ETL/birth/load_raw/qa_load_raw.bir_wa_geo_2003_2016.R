...
# Code to QA load_raw.bir_wa_geo_2003_2016

###############################################################################


qa_load_raw_bir_wa_geo_2003_2016_f <- function(conn = db_apde,
                                               load_only = F) {
  ...
  row_count_tot <- as.numeric(dbGetQuery(conn, 
                                         "SELECT COUNT (*) FROM load_raw.bir_wa_geo_2003_2016"))
  ...
  etl_batch_id_max <- as.numeric(dbGetQuery(conn, "SELECT MAX (etl_batch_id) FROM load_raw.bir_wa_geo_2003_2016"))
      ...
      dbGetQuery(conn, 
                 "SELECT a.qa_value FROM
                       (SELECT * FROM metadata.qa_bir_values
                         WHERE table_name = 'load_raw.bir_wa_geo_2003_2016' AND
  ...
  #### RANGE OF YEARS ####
  min_yr <- min(row_count_yr$dob_yr)
  max_yr <- max(row_count_yr$dob_yr)
  ...
  cert_count <- as.numeric(dbGetQuery(conn, 
                                      "SELECT COUNT (DISTINCT certno_e) 
                                            FROM load_raw.bir_wa_geo_2003_2016"))
    ...
    dbGetQuery(
      ...
      glue_sql("INSERT INTO metadata.qa_bir
                   ...
                   VALUES ({etl_batch_id_max}, 
                           ...
                           'FAIL', 
                           {Sys.time()}, 
                           'Row counts fell outside the expected range (25-40k) for the following years: {row_count_yr_outlier$dob_yr*} 
                           (maximum etl_batch_id shown, range {etl_batch_id_min}:{etl_batch_id_max})')",
               .con = conn))
  ...
  load_sql <- glue_sql("INSERT INTO metadata.qa_bir_values
                             ...
                             VALUES ('load_raw.bir_wa_geo_2003_2016',
                                     '2003-01-01',
                                     '2016-12-31',
                                     'row_count', 
                                     {row_count_tot}, 
