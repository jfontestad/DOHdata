...
#### SET UP PATH to SQL OUTPUT TABLE ----
    tbl_id_2003_20xx <- DBI::Id(schema = yaml_config$schema,
                            table = yaml_config$table)
...
#### Run sql_clean() ----
  sql_clean(bir_combined) # trims white space from both ends and changes empty strings to NA
...
#### Process simple recodes with package ----
  birth.recodes <- rads::parse_recode_instructions(recodes)
  bir_combined <- rads::enact_recodes(data = bir_combined, birth.recodes, ignore_case = T)
...
#### Custom code for complex recoding ----
  ...
  # bw_norm_sing ----
    bir_combined[!(is.na(bw_norm) | is.na(singleton)), bw_norm_sing := 0]
    bir_combined[bw_norm == 1 & singleton == 1, bw_norm_sing := 1]
    bir_combined[, .N, (bw_norm_sing)]

  # bw_low_sing ----
    bir_combined[!(is.na(bw_low) | is.na(singleton)), bw_low_sing := 0]
    bir_combined[bw_low == 1 & singleton == 1, bw_low_sing := 1]
    bir_combined[, .N, (bw_low_sing)]

  # bw_vlow_sing ----
    bir_combined[!(is.na(bw_vlow) | is.na(singleton)), bw_vlow_sing := 0]
    bir_combined[bw_vlow == 1 & singleton == 1, bw_vlow_sing := 1]
    bir_combined[, .N, (bw_vlow_sing)]

  # chi_geo_zip5  ----
    bir_combined[, chi_geo_zip5 := mother_residence_zip]
    bir_combined[chi_geo_zip5 == "99999", chi_geo_zip5 := NA]
    bir_combined[, .N, (chi_geo_zip5)]
  ...
  # cigarettes_smoked_3_months_prior ----
    ...
    bir_combined[, .N, (cigarettes_smoked_3_months_prior)]

  # smokeprior ----
    bir_combined[cigarettes_smoked_3_months_prior == 0, smokeprior := 0] # partially recoded with recode function above
    bir_combined[, .N, (smokeprior)]
  ...
  # chi_race_aic_asian ----
    ...
    bir_combined[, .N, (chi_race_aic_asian)]

  # chi_race_eth7 -----
    bir_combined[mother_race_summary_code == 12, chi_race_eth7 := "AIAN"]
    bir_combined[mother_race_summary_code == 13, chi_race_eth7 := "Asian"]
    bir_combined[mother_race_summary_code == 11, chi_race_eth7 := "Black"]
    bir_combined[mother_race_summary_code %between% c(20, 50), chi_race_eth7 := "Multiple"]
    bir_combined[mother_race_summary_code == 14, chi_race_eth7 := "NHPI"]
    bir_combined[mother_race_summary_code == 10, chi_race_eth7 := "White"]
    bir_combined[chi_race_hisp == 1, chi_race_eth7 := "Hispanic"]

    bir_combined[, table(chi_race_eth7, chi_race_hisp, exclude = F)]
    bir_combined[, table(chi_race_eth7, mother_race_summary_code, exclude = F)]

  # chi_race_eth8 ----
    bir_combined[mother_race_summary_code == 12, chi_race_eth8 := "AIAN"]
    bir_combined[mother_race_summary_code == 13, chi_race_eth8 := "Asian"]
    bir_combined[mother_race_summary_code == 11, chi_race_eth8 := "Black"]
    bir_combined[mother_race_summary_code %between% c(20, 50), chi_race_eth8 := "Multiple"]
    bir_combined[mother_race_summary_code == 14, chi_race_eth8 := "NHPI"]
    bir_combined[mother_race_summary_code == 10, chi_race_eth8 := "White"]
    bir_combined[mother_race_summary_code == 99, chi_race_eth8 := "Oth/unk"]
    bir_combined[is.na(mother_race_summary_code), chi_race_eth8 := "Oth/unk"]
    bir_combined[chi_race_hisp == 1, chi_race_eth8 := "Hispanic"]

    bir_combined[, table(chi_race_eth8, chi_race_hisp, exclude = F)]
    bir_combined[, table(chi_race_eth8, mother_race_summary_code, exclude = F)]
  ...
  # mother_birthplace_usa ----
    ...
    bir_combined[, .N, (mother_birthplace_usa)]
  ...
  # mother_birthplace_foreign ----
    ...
    bir_combined[, .N, (mother_birthplace_foreign)]
  ...
  # ch_priorpreg ----
    ...
    bir_combined[, .N, (ch_priorpreg)]

  # smoking (Smoking-Yes (before &|or during pregnancy)) ----
    if("smoking" %in% names(bir_combined)){bir_combined[, smoking := NULL]}
...
# DROP USELESS COLUMNS
    if("blankblank" %in% names(bir_combined)){bir_combined[, blankblank := NULL]}
    if("res_geo_census_full_2010" %in% names(bir_combined)){bir_combined[, res_geo_census_full_2010 := NULL]}
...
#### LOAD TO SQL ####
  ...
  ## Ensure col type/class in R matches SQL ----
        ...
        yaml.table <- data.table(
          name =  c(names(yaml_config$vars), names(yaml_config$recodes)),
          yaml.class = c(as.character(yaml_config$vars), as.character(yaml_config$recodes)))

          yaml.table[, yaml.class := tolower(yaml.class)]
          yaml.table[yaml.class %like% "char", yaml.class := "character"]
          yaml.table[yaml.class %in% c("integer", "tinyint"), yaml.class := "numeric"]
