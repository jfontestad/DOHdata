## Header ####
    ...
    #
    # Notes: did not check all variables vis-?-vis CHAT, but rather a random assorment
...
## Load staged from SQL ----
    ...
    if(!exists("bir_combined")){ # only load data if needed
      query.varlist <- paste(c(names(yaml_config$vars), names(yaml_config$recodes)), collapse = ", ")
...
## Identify numeric columns ----
    numerics <- data.table(name = names(yaml_config$vars), type = as.character(yaml_config$vars))
...
## COMPARE TO CHAT ----
    ...
    # Race compared to CHAT ----
        ...
        setnames(race.doh, "race3", "chi_race_7")
    ...
    # Hispanic compared to CHAT ----
        ...
        hispanic <- hispanic[chi_race_hisp==1]
