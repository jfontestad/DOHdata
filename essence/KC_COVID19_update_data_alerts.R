    ...
    if (condition %in% c("cli","ili") &
      ...
      comb2 <- combined %>%
        ...
        mutate(cnt3day = zoo::rollmean(cnt, 3, na.pad = T, align = "right"),
               sum3day = zoo::rollsum(cnt, 3, na.pad = T, align = "right"),
               tot3day = zoo::rollsum(tot, 3, na.pad = T, align = "right")
        ) %>%
        mutate(pct3day = sum3day/tot3day) %>%
        select(-sum3day, -tot3day) %>%
